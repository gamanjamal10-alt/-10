...
export const TaskList: React.FC<TaskListProps> = ({ tasks, onTaskClick, onViewAllClick }) => {
    return (
        <div className="flex flex-col gap-4 rounded-xl p-4 bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark">
            <div className="flex justify-between items-center">
                <h3 className="text-text-light-primary dark:text-dark-primary text-lg font-bold leading-tight tracking-[-0.015em]">المهام القادمة</h3>
                <a className="text-primary text-sm font-medium hover:underline cursor-pointer" onClick={onViewAllClick}>عرض الكل</a>
            </div>
            <div className="flex flex-col gap-3">
                {tasks.map((task, index) => <TaskItem key={index} {...task} onClick={() => onTaskClick(task)} />)}
            </div>
        </div>
    );
};